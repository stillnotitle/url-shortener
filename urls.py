...
def shorten_url_with_campaign():
  ...
  campaign_source = st.text_input(
      "utm_source（必須）",
      help="プロパティにトラフィックを誘導した広告主、サイト、出版物、その他を識別します（Google、ニュースレター 4、屋外広告など）。")
  campaign_medium = st.text_input(
      "utm_medium（必須）",
      help="広告メディアやマーケティング メディアを識別します（CPC 広告、バナー、メール ニュースレターなど）。")
  campaign_name = st.text_input("utm_campaign（必須）",
                                help="商品のキャンペーン名、テーマ、プロモーション コードなどを指定します。")
